                ...
                <div class="col-lg-8 col-xl-6">
                    <div class="card rounded-3">
                        <div class="card-body p-4 p-md-5">
                            <h3 class="pb-md-0 mb-md-5 px-md-2">Update Dependent Details</h3>
                                        ...
                                        if ($check_result->num_rows === 0) {
                                            ...
                                            try {
                                                ...
                                                if (isset($_POST['checkmother']) && !empty($_POST['txtnicmother']) && !empty($_POST['txtFnamemother'])) {
                                                    $stmt = $con->prepare("UPDATE dependent SET NIC = ?, name = ? WHERE Aid = ? AND dependent_type = 'mother'");
                                                    $stmt->bind_param("sss", $_POST['txtnicmother'], $_POST['txtFnamemother'], $Aid);
                                                    if ($stmt->execute()) {
                                                        if ($stmt->affected_rows > 0) {
                                                            $successCount++;
                                                        } else {
                                                            $errorMessages[] = "Mother: No record found to update or data unchanged";
                                                        }
                                                    } else {
                                                ...
                                                if (isset($_POST['checkfather']) && !empty($_POST['txtnicfather']) && !empty($_POST['txtFnamefather'])) {
                                                    $stmt = $con->prepare("UPDATE dependent SET NIC = ?, name = ? WHERE Aid = ? AND dependent_type = 'father'");
                                                    $stmt->bind_param("sss", $_POST['txtnicfather'], $_POST['txtFnamefather'], $Aid);
                                                    if ($stmt->execute()) {
                                                        if ($stmt->affected_rows > 0) {
                                                            $successCount++;
                                                        } else {
                                                            $errorMessages[] = "Father: No record found to update or data unchanged";
                                                        }
                                                    } else {
                                                ...
                                                if (isset($_POST['checkspouse']) && !empty($_POST['txtnicspouse']) && !empty($_POST['txtFnamespouse'])) {
                                                    $stmt = $con->prepare("UPDATE dependent SET NIC = ?, name = ? WHERE Aid = ? AND dependent_type = 'spouse'");
                                                    $stmt->bind_param("sss", $_POST['txtnicspouse'], $_POST['txtFnamespouse'], $Aid);
                                                    if ($stmt->execute()) {
                                                        if ($stmt->affected_rows > 0) {
                                                            $successCount++;
                                                        } else {
                                                            $errorMessages[] = "Spouse: No record found to update or data unchanged";
                                                        }
                                                    } else {
                                                ...
                                                // Check if no dependents were updated
                                                if ($successCount === 0 && empty($errorMessages)) {
                                                    echo "<div class='alert alert-warning'>No dependents were selected for update or no changes were made.</div>";
                                                } else {
                                                    // Commit transaction if all updates were successful
                                                    $con->commit();
                                                    
                                                    if ($successCount > 0) {
                                                        echo "<div class='alert alert-success'>Successfully updated $successCount dependent(s).</div>";
                                                    }
